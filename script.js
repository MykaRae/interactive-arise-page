...
// This object simulates patient EHR data used when showing a room's
// patient snapshot. In production this would be fetched from an
// EHR/API integration (e.g., athenahealth) keyed by patient ID.
const ehrRecords = {
	p1: {
		name: "Ruth Garrett",
		...
		recentVisits: [
			"2025-04-15: Hypertension follow-up, BP 148/90",
			"2025-03-20: Lipid panel, LDL 130",
		],
		labResults: ["04/15: LDL 115", "Creatinine 0.9"],
		careTeam: ["Dr. Aris", "Elena"],
	},
	p2: {
		name: "Dennis Okonkwo",
		allergies: ["None"],
		medications: ["Ibuprofen 400mg PRN", "Metformin 500mg BID"],
		recentVisits: [
			"2025-04-10: Knee pain follow-up",
			"2025-03-01: Annual physical",
		],
		labResults: ["A1c 6.1", "Vitamin D 28"],
		careTeam: ["Dr. Aris", "Physical Therapy"],
	},
	p3: {
		name: "J. Miller",
		allergies: ["None"],
		medications: [],
		recentVisits: ["2025-04-20: Skin biopsy scheduled"],
		labResults: ["Pathology pending"],
		careTeam: ["Dr. Aris"],
	},
	p4: {
		name: "Eleanor Hughes",
		allergies: ["Shellfish (hives)"],
		medications: ["Levothyroxine 50mcg daily", "Losartan 25mg"],
		recentVisits: [
			"2025-04-12: Hypothyroidism follow-up",
			"2025-03-25: BP check",
		],
		labResults: ["TSH 2.1 (normal)", "BMP normal"],
		careTeam: ["Dr. Aris"],
	},
	p5: {
		name: "George Baxter",
		...
		recentVisits: [
			"2025-04-18: Hypertension follow-up",
			"2025-04-01: Lipid panel",
		],
		labResults: ["LDL 95", "Triglycerides 150"],
		careTeam: ["Dr. Aris", "Cardiology"],
	},
	p6: {
		name: "Mildred Shaw",
		allergies: ["Codeine (nausea)"],
		medications: ["Albuterol inhaler PRN", "Omeprazole 20mg"],
		recentVisits: [
			"2025-04-14: Asthma follow-up",
			"2025-03-10: GERD assessment",
		],
		labResults: ["Peak flow 85%", "Chest X-ray clear"],
		careTeam: ["Dr. Aris"],
	},
};

// Scheduled arrivals (NOT checked in) - distinct names
// Lobby lists: patients scheduled but not yet checked in, and
// those who have checked in and are waiting for room assignment.
let notCheckedIn = [
...
// Rooms array models the clinic exam rooms and their current
// occupancy/status. Each room holds metadata used by the HUD UI.
let rooms = [
	{
		id: 1,
		name: "Exam 1",
...
// Tracks the most recent user action to support undo functionality.
let lastAction = null;

// Ordering and display names for room grouping in the UI. Keeping
// these centralized makes rendering and status changes consistent.
...
// Periodic timer used to:
// - Flag checked-in patients who have waited longer than the threshold
// - Start/stop per-room ready timers and set provider-wait alerts
// It updates model state; UI is re-rendered via renderAll().
function updateTimers() {
	const CHECKIN_WAIT_THRESHOLD = 15 * 60 * 1000; // 15 minutes
	const READY_WAIT_THRESHOLD = 5 * 60 * 1000; // 5 minutes
	const now = Date.now();
	// Update waiting alerts for checked-in patients
	checkedInWaiting.forEach((p) => {
		if (p.checkinTime && now - p.checkinTime > CHECKIN_WAIT_THRESHOLD)
			p.alert = true;
		else p.alert = false;
	});
	// Update per-room ready timers/alerts
	rooms.forEach((room) => {
		if (room.status === "ready") {
			if (!room.readyTimerStart) room.readyTimerStart = Date.now();
			room.readyAlert =
				Date.now() - room.readyTimerStart > READY_WAIT_THRESHOLD;
		} else {
...
function renderLobbySections() {
	...
	// Render scheduled-but-not-checked-in list with Check In buttons
	...
	// Render checked-in/waiting list with Assign to Room buttons
	checkedInWaiting.forEach((p) => {
		const row = document.createElement("div");
		row.className = "patient-row";
...
// Event delegation for lobby actions: check-in and assign-to-room.
// Using delegation keeps markup simple and attaches a single listener.
...
document
	.getElementById("checkedInWaitingList")
	.addEventListener("click", (e) => {
		// Assign a checked-in patient to the first available clean room
		...
		if (patient) {
			...
			if (cleanRoom) {
				// Move patient into the clean room and update metadata
...
function renderGroupedRooms() {
	...
	for (let status of statusOrder) {
		...
		// Render each room card inside the appropriate status group
		groupRooms.forEach((room) => {
			...
			// Clicking a room card opens the snapshot panel for that room
...
function getEhrSummary(patientId) {
	...
	if (!rec)
		return '<div class="ehr-section">No EHR data available for this patient.</div>';
	return `
...
// Simulated action to open the full EHR/chart for a given patient ID.
// In a real integration this would redirect to an external EHR URL.
function openPatientChart(patientId) {
	const patient = ehrRecords[patientId];
	if (patient) {
		showToast(
			`Opening chart for ${patient.name}... (demo: would redirect to EHR)`,
		);
		// Uncomment for actual redirect: window.open(`https://clinic-ehr.example.com/patient/${patientId}`, '_blank');
		alert(
			`[Demo] Opening full EHR chart for ${patient.name}\nPatient ID: ${patientId}\nIn production, this would link to your EHR system.`,
		);
	} else {
...
function showSnapshot(room) {
	...
	// Build the snapshot panel contents based on room state. There are
	// separate UI flows for: vacant/clean rooms, dirty rooms, and
	// occupied rooms (in-progress/ready/procedure) which show patient
	// details and EHR summary.
	...
	if (room.status === "ready" && room.readyAlert) {
		alertMsg =
			'<div class="timer-alert" style="color:#FFB700;">⚠️ Provider delay alert – patient waiting >5 min</div>';
	}
	// Include EHR summary HTML when a patientId is present
	const ehrHtml = room.patientId
		? getEhrSummary(room.patientId)
		: '<div class="ehr-section">No EHR record linked.</div>';
	...
	// Wire up action buttons inside the snapshot (view chart, apply
	// status, mark ready/dirty). Handlers update model state and re-
	// render the UI.
	const chartBtn = document.getElementById("viewChartBtn");
	if (chartBtn && room.patientId)
		chartBtn.onclick = () => openPatientChart(room.patientId);
	const applyBtn = document.getElementById("applyStatusBtn");
	if (applyBtn)
		applyBtn.onclick = () => {
			const newStatus = document.getElementById("statusSelect").value;
...
function changeRoomStatus(room, newStatus) {
	// Change a room's status and update related fields (patient
	// assignment, timers, glyph). This centralizes all status
	// transitions so the UI and undo stack remain consistent.
	...
	if (newStatus === "clean" || newStatus === "dirty") {
		...
		(newStatus === "in-progress" || newStatus === "ready") &&
		!room.patient
	) {
		room.patient = {
			name: "Unknown",
...
// Undo handler: restores the model to the state before lastAction.
// Supports undoing status changes, check-ins, and room assignments.
document.getElementById("undoBtnTop").onclick = () => {
	if (lastAction?.type === "statusChange") {
		...
		if (room) {
			...
			room.readyTimerStart =
				lastAction.oldStatus === "ready" ? Date.now() : null;
			room.readyAlert = false;
			renderAll();
...
// Kick off periodic timers: updateTimers updates alerts and waits;
// updateClock refreshes the HUD clock display every second.
...
// Reports modal: show clinic metrics and allow exporting CSVs.
...
function generateReportData() {
	// Populate the simple stats shown in the reports modal. In a
	// production version these numbers would be calculated from real
	// telemetry and visit records.
	...
	document.getElementById("exportDailyCSV").onclick = () =>
		downloadCSV("Date,Patients\n2025-04-30,22", "daily.csv");
}
